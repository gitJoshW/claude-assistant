...
Single table `store` with columns `(key TEXT PRIMARY KEY, value TEXT, version INTEGER, updated_at TIMESTAMPTZ)`.
Everything is stored as a JSON string under a string key. `version` increments on every
write to a key (via `dbSet` or `dbSetVersioned`) and backs the optimistic-concurrency
check described under Known Quirks below.
...
Helper functions `dbGet(key)` and `dbSet(key, value)` handle unversioned DB access
(used by schedulers/webhooks). `dbGetVersioned(key)` and `dbSetVersioned(key, value, expectedVersion)`
back the five versioned collection endpoints — see Known Quirks below.
...
|---|---|---|
| GET | `/api/tasks` | `{ tasks, version }` |
| POST | `/api/tasks` | Save all tasks (full replace) — body `{ tasks, version }`, versioned |
| GET | `/api/goals` | `{ goals, version }` |
| POST | `/api/goals` | Save all goals — body `{ goals, version }`, versioned |
| GET | `/api/projects` | `{ projects, version }` |
| POST | `/api/projects` | Save all projects — body `{ projects, version }`, versioned |
| GET | `/api/notes` | `{ notes, version }` |
| POST | `/api/notes` | Save all notes — body `{ notes, version }`, versioned |
| GET | `/api/completion-log` | `{ log, version }` |
| POST | `/api/completion-log` | Save completion log — body `{ log, version }`, versioned |
| GET | `/api/memory` | Get memory string (not versioned) |
| POST | `/api/memory` | Save memory string (not versioned) |
| GET | `/api/notifications` | Get notification log (last 20, not versioned) |
| POST | `/api/notifications/save` | Save/overwrite notification log (not versioned) |

**Versioned endpoints** (tasks/goals/projects/notes/completion-log): the `store`
table has a `version` column that increments on every write to a key. POST
must include the `version` last read via GET; if it doesn't match the current
DB version, the write is rejected with `409 { error: 'conflict', <field>: current, version }`
instead of silently overwriting. Pass `force: true` in the POST body to skip
the check and write unconditionally (used by `importData()` for backup restores).
Every write also logs an `[AUDIT] <key>: N -> M items | +added | -removed`
line (by item `id`) to stdout — check Railway logs to see what changed.
...
- **Full replace on save, with a version guard** — all POST endpoints replace the entire
  array (no partial update — always send the complete current array). The five synced
  collections (tasks/goals/projects/notes/completion-log) are also version-checked: a
  save whose `version` doesn't match the DB's current version is rejected with 409
  rather than silently overwriting newer data. This exists because a stale browser tab
  open on another device once silently deleted a task this way. Frontend saves must go
  through `saveTasks()`/`saveGoals()`/`saveProjects()`/`saveNotes()`/`saveCompletionLog()`
  (not raw `apiFetch('POST', '/api/tasks', ...)`) so the version is tracked and 409s are
  handled (re-sync + alert the user to redo their edit).
